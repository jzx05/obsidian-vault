...
1. **ImageWAM 真正送给 Action Expert 的是什么？** 最终编辑图、图像 latent，还是逐层 KV cache？为什么作者不直接使用最终图？
   ...
   - ![[Pasted image 20260712161528.png]]
1. **论文中的"未来目标图"是整项任务完成后的图像，还是固定 $H$ 帧后的图像？** 这个区别为什么重要？
   - → 它不是整项任务完成后的最终图像，而是固定 horizon 后的短期未来图。
1. **图像编辑 flow time 和动作 flow time 是同一个变量吗？** 它们分别控制什么？
...
| 论文 | 核心思路 | 备注 |
|------|----------|------|
| DreamZero | 最简单的默认做法：添加动作标记（连续或离散）和动作头，动作被视为与视频并列的另一种模态 | baseline 做法 |
| GENIMA | 动作即图像——动作被表达为生成图像模型可以绘制的内容 | 接口选择有趣 |
| Cosmos Policy | GENIMA 的进阶版本 | |

### 潜在行动与计划（Latent Action & Planning）
| 论文 | 核心思路 | 备注 |
|------|----------|------|
| Play-LMP (2019) | 早期潜在动作计划方法 | 经典工作 |
| Genie | 潜在动作空间学习 | |
| Being-H0.7 | 展示了如何在现代 WAM/VLA 混合模型中扩展潜在动作接口 | |
...
| 论文 | 核心思路 | 备注 |
|------|----------|------|
| V-JEPA 2 | 直接从预训练潜在空间中的视频学习潜在动态；相比扩散视频生成，更低部署成本、更快推理、更清晰规划信号 | 正交方向，值得关注 |
| VLA-JEPA | 该方向的首批 WAM 模型之一 | |
| Being-H0.7 | 同上，潜在世界模型方向的早期 WAM | 与上面重叠 |
